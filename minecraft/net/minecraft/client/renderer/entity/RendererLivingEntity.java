...
import net.AzureWare.mod.mods.RENDER.AntiAim;
import net.AzureWare.mod.mods.RENDER.Chams;
import net.AzureWare.mod.mods.RENDER.ESP;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
		...
		if (!Reflector.RenderLivingEvent_Pre_Constructor.exists()
			...
			try {
				...
				if (this.renderOutlines) {
		            ...
		            if (AntiAim.renderEntity.getValueState() && entity.getHealth() > 0.0 && entity instanceof EntityPlayer && (entity instanceof EntityPlayerSP))
		            {
		                GL11.glPushMatrix();
	 ...
	 */
	public boolean isVaild(T entitylivingbaseIn) {
		if (Chams.monster.getValueState() && Chams.animals.getValueState() && entitylivingbaseIn instanceof Entity) {
			return true;
		}
		if (Chams.monster.getValueState()
				&& (entitylivingbaseIn instanceof EntityMob || entitylivingbaseIn instanceof EntityPlayer)) {
			return true;
		}
		if (Chams.animals.getValueState() && entitylivingbaseIn instanceof EntityAnimal
				|| entitylivingbaseIn instanceof EntityPlayer) {
			return true;
		}
		if (!Chams.monster.getValueState() && !Chams.animals.getValueState()
				&& entitylivingbaseIn instanceof EntityPlayer) {
			return true;
		}
		return false;
	...
	protected void renderModel(T entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_,
		...
		if (flag || flag1) {
			...
			if (!(entitylivingbaseIn instanceof EntityPlayerSP) && ESP.mode.isCurrentMode("HotRender")
					&& ModManager.getModByName("ESP").isEnabled()) {
				// String s = "by navex";
				GL11.glPushMatrix();
				GL11.glDisable(2896);
				GL11.glDisable(3553);
			...
			if (ModManager.getModByName("Chams").isEnabled() && Chams.invisable.getValueState()
					&& (isVaild(entitylivingbaseIn))
				...
				GL11.glEnable(GL11.GL_BLEND);
				GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_CONSTANT_ALPHA);
				GL11.glColor3f(Chams.red.getValueState().floatValue() / 100,
						Chams.green.getValueState().floatValue() / 100, Chams.blue.getValueState().floatValue() / 100);
				this.MainModel.render(entitylivingbaseIn, p_77036_2_, p_77036_3_, p_77036_4_, p_77036_5_, p_77036_6_,
						p_77036_7_);
				GL11.glPopAttrib();
				GL11.glPopMatrix();
			...
			if (ModManager.getModByName("Chams").isEnabled()) {
				if (Chams.emt.getValueState()) {
					this.MainModel.render(entitylivingbaseIn, p_77036_2_, p_77036_3_, p_77036_4_, p_77036_5_,
							p_77036_6_, p_77036_7_);
				} else {
					if ((entitylivingbaseIn == Minecraft.thePlayer))
					this.MainModel.render(entitylivingbaseIn, p_77036_2_, p_77036_3_, p_77036_4_, p_77036_5_,
							p_77036_6_, p_77036_7_);
				}
	...
	public void renderName(T entity, double x, double y, double z) {
		if (entity instanceof EntityPlayer && ModManager.getModByName("NameTag").isEnabled())
			return;
