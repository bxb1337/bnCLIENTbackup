...
import net.AzureWare.mod.mods.RENDER.HitAnimation;
...
public class ItemRenderer {
	...
	private void doItemUsedTransformations(float swingProgress) {
		if (ModManager.getModByName("HitAnimation").isEnabled() && HitAnimation.Smooth.getValueState()) {
			float var2 = -0.15f * MathHelper.sin(MathHelper.sqrt_float(swingProgress) * 3.1415927f);
			float var3 = -0.05f * MathHelper.sin(MathHelper.sqrt_float(swingProgress) * 3.1415927f * 1.5f);
			float var4 = -0.0f * MathHelper.sin(swingProgress * 3.1415927f);
	...
	public void renderItemInFirstPerson(float partialTicks) {
		...
		float f22 = abstractclientplayer.prevRotationPitch
		...
		GlStateManager.pushMatrix();
		HitAnimation hud = (HitAnimation) ModManager.getModByName("HitAnimation");
		if (this.itemToRender != null) {
			...
			if (this.itemToRender.getItem() == Items.filled_map) {
				...
				case 4:
					byte var5 = 0;
					HitAnimation Ani = (HitAnimation) ModManager.getModByName("HitAnimation");
					GlStateManager.translate(hud.x.getValueState(), hud.y.getValueState(), hud.z.getValueState());
					if (!Ani.isEnabled()) {
						this.transformFirstPersonItem(af / 2.0F, af1);
						this.doBlockTransformations();
						break;
					} else {
						float var2 = 1.0F - (this.prevEquippedProgress
						...
						if (Ani.mode.getModeName() == "Swang") {
							var5 = 0;
						}
						if (Ani.mode.getModeName() == "Swank") {
							var5 = 1;
						}

						if (Ani.mode.getModeName() == "Swing") {
							var5 = 2;
						}

						if (Ani.mode.getModeName() == "Swong") {
							var5 = 3;
						}

						if (Ani.mode.getModeName() == "SwAing") {
							var5 = 4;
						}
						if (Ani.mode.getModeName() == "Remix") {
							var5 = 5;
						}
						if (Ani.mode.getModeName() == "Custom") {
							var5 = 6;
						}
						if (Ani.mode.getModeName() == "None") {
							var5 = 7;
						}
						if (Ani.mode.getModeName() == "Old") {
							var5 = 8;
						}
						if (Ani.mode.getModeName() == "Gay") {
							var5 = 9;
						}
						if (Ani.mode.getModeName() == "Punch") {
							var5 = 10;
						}
						if (Ani.mode.getModeName() == "Winter") {
							var5 = 11;
						}
						if (Ani.mode.getModeName() == "rotate") {
							var5 = 12;
						}
						if (Ani.mode.getModeName() == "circle") {
							var5 = 13;
						}
						...
						case 1:
							GL11.glTranslated(-0.1F, 0.15F, 0.0F);
							this.transformFirstPersonItem(af / 2.0F, af1);
							var15 = MathHelper.sin(MathHelper.sqrt_float(af1) * 3.1415927F);
						...
						case 3:
							...
							this.transformFirstPersonItem(af / 2.0F, 0.0f);
							var15 = MathHelper.sin(MathHelper.sqrt_float(af1) * 3.1415927F);
							GL11.glRotated(-var15 * 40.0F / 2.0F, var15 / 2.0F, -0.0F, 9.0F);
							GL11.glRotated(-var15 * 30.0F, 1.0F, var15 / 2.0F, -0.0F);
							this.doBlockTransformations();
							break;
						case 4:
							GL11.glTranslated(-0.1F, 0.15F, 0.0F);
						...
						case 5:
							...
							this.doBlockTransformations();
							float f4 = MathHelper.sin((float) (MathHelper.sqrt_float((float) f1) * 3.83f));
							GlStateManager.translate((float) -0.5f, (float) 0.2f, (float) 0.2f);
							GlStateManager.rotate((float) (-f4 * 0.0f), (float) 0.0f, (float) 0.0f, (float) 0.0f);
							GlStateManager.rotate((float) (-f4 * 43.0f), (float) 58.0f, (float) 23.0f, (float) 45.0f);
							break;
						case 6:
				...
				byte var5 = 0;
				HitAnimation Ani = (HitAnimation) ModManager.getModByName("HitAnimation");
				if (hud.EveryThingBlock.getValueState() && mc.gameSettings.keyBindUseItem.isKeyDown()) {
					GL11.glTranslated(hud.x.getValueState(), hud.y.getValueState(), hud.z.getValueState());
					if (!Ani.isEnabled()) {
						this.transformFirstPersonItem(af / 2.0F, af1);
						this.doBlockTransformations();
					} else {
						...
						if (!Ani.isEnabled()) {
							...
							case 5:
								...
								GlStateManager.translate((float) -0.5f, (float) 0.2f, (float) 0.2f);
								GlStateManager.rotate((float) (-f4 * 0.0f), (float) 0.0f, (float) 0.0f, (float) 0.0f);
								GlStateManager.rotate((float) (-f4 * 43.0f), (float) 58.0f, (float) 23.0f,
										(float) 45.0f);
								break;
							case 6:
								this.func_178096_b(f2, 0.0F);
	...
	private void circle(float swingProgress) {
		...
		GlStateManager.rotate(ticks * 0.2f * HitAnimation.Speed.getValue().floatValue(), 0, 0, 1);
	...
	public void renderOverlays(float partialTicks) {
		...
		if (this.mc.thePlayer.isEntityInsideOpaqueBlock()) {
			...
			if (iblockstate.getBlock().getRenderType() != -1) {
				...
				if (!Reflector.callBoolean(Reflector.ForgeEventFactory_renderBlockOverlay, this.mc.thePlayer,
						partialTicks, object, iblockstate, blockpos)) {
		...
		if (!this.mc.thePlayer.isSpectator()) {
			if (this.mc.thePlayer.isInsideOfMaterial(Material.water) && !Reflector
					.callBoolean(Reflector.ForgeEventFactory_renderWaterOverlay, this.mc.thePlayer, partialTicks)) {
				this.renderWaterOverlayTexture(partialTicks);
			}

			if (this.mc.thePlayer.isBurning() && !Reflector.callBoolean(Reflector.ForgeEventFactory_renderFireOverlay,
					this.mc.thePlayer, partialTicks)) {
	...
	private void renderFireInFirstPerson(float partialTicks) {
		if (!HitAnimation.NoFire.getValueState()) {
			Tessellator tessellator = Tessellator.getInstance();
			WorldRenderer worldrenderer = tessellator.getWorldRenderer();
	...
	public void updateEquippedItem() {
		...
		if (this.itemToRender != null && itemstack != null) {
			if (HitAnimation.Eliminates.getValueState() ? !this.itemToRender.getIsItemStackEqual2(itemstack)
					: !this.itemToRender.getIsItemStackEqual(itemstack)) {
				flag = true;
			}
		} else {
...
//        float f1 = flag?0.0F:1.0F;
		...
		float f22 = MathHelper.clamp_float(
				((Killaura.target != null && Killaura.target.hurtTime > HitAnimation.HurtTime.getValueState().intValue()
						&& !HitAnimation.Eliminates.getValueState() || flag) ? 0.0F : 1.0F) - this.equippedProgress,
				-f2, f2);
