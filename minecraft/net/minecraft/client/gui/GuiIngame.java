...
import net.AzureWare.mod.mods.WORLD.AutoGG;
...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        //Event 
        EventManager.call(new EventRenderGui(partialTicks));
        ...
        if (this.field_175195_w > 0)
        {
            ...
            if (l1 > 8)
            {
                ...
                GlStateManager.popMatrix();
                try {
                    if(ModManager.getModByName("AutoGG").isEnabled()){
                        if(ggTimer.isDelayComplete(10000) && this.field_175201_x.contains("¡ìr¡ì6¡ìl")) {
                        	mc.thePlayer.sendChatMessage("/ac GG" + (AutoGG.ad.getValueState() ? " ¹ºÂòAzureWareµØÖ·£ºmcheika.com" : ""));
                        	ggTimer.reset();
                        }
                    }
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_)
    {
        ...
        for (Object score1 : arraylist1)
        {
            ...
            int l = k1 - k * this.getFontRenderer().FONT_HEIGHT + p_180475_2_.getScaledHeight() - k1 - 70;
            //System.out.println(l);
            int i1 = p_180475_2_.getScaledWidth() - b0 + 2;
            
            if(s1.contains("www.hypixel")) {
            	Client.Server = "Hypixel";
            }else if(s1.contains("163") && s1.contains("hyp")) {
            	Client.Server = "HypixelCN";
            }
            
            if (s1.contains("hyp")) s1 = "\247eAzureWare.net";
            ...
            if (k == arraylist1.size())
            {
                ...
                Client.playingGame = s3.replaceAll("\247.", "");
            }
        }
    }
