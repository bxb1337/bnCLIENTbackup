...
import net.AzureWare.mod.mods.MOVEMENT.NoSlow;
import net.AzureWare.mod.mods.RENDER.Particles;
...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public void moveEntity(double x, double y, double z) {
		...
		EventUpdate event1 = new EventUpdate();
	...
	public void onUpdateWalkingPlayer() {
		...
		EventPreMotion e = new EventPreMotion(rotationYaw, rotationPitch, posX, posY, posZ, mc.thePlayer.onGround);
		// EventPostMotionUpdates post = new EventPostMotionUpdates(rotationYaw,
		// rotationPitch);
		EventManager.call(e);

		if (e.isCancelled()) {
			EventPostMotion post = new EventPostMotion(rotationYaw, rotationPitch, posX, posY, posZ, mc.thePlayer.onGround);
		...
		if (this.isCurrentViewEntity()) {
			...
			if (this.ridingEntity == null) {
				if (flag2 && flag3) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.posX, e.y, this.posZ,
							e.yaw, e.pitch, e.isGround()));
				} else if (flag2) {
					this.sendQueue.addToSendQueue(
							new C03PacketPlayer.C04PacketPlayerPosition(this.posX, e.y, this.posZ, e.isGround()));
				} else if (flag3) {
					this.sendQueue
							.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(e.yaw, e.pitch, e.isGround()));
				} else {
				...
				this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0,
						this.motionZ, e.yaw, e.pitch, e.isGround()));
				flag2 = false;
			}
	...
	public void onCriticalHit(Entity entityHit) {
		...
		if (ModManager.getModByName("Particles").isEnabled() && Particles.mode.isCurrentMode("SwordArt")) {
			this.mc.effectRenderer.emitParticleAtEntity(entityHit, EnumParticleTypes.HEART);
			this.mc.effectRenderer.emitParticleAtEntity(entityHit, EnumParticleTypes.ENCHANTMENT_TABLE);
		}
		if (ModManager.getModByName("Particles").isEnabled() && Particles.mode.isCurrentMode("EXPLOSION")) {
			this.mc.effectRenderer.emitParticleAtEntity(entityHit, EnumParticleTypes.EXPLOSION_LARGE);

		}
		if (ModManager.getModByName("Particles").isEnabled() && Particles.mode.isCurrentMode("Flame")) {
			this.mc.effectRenderer.emitParticleAtEntity(entityHit, EnumParticleTypes.FLAME);
			this.mc.effectRenderer.emitParticleAtEntity(entityHit, EnumParticleTypes.WATER_BUBBLE);
	...
	public void onLivingUpdate() {
		...
		if (this.isUsingItem() && !this.isRiding() && !ModManager.getModByName("NoSlow").isEnabled()) {
		...
		if (this.isUsingItem() && !this.isRiding() && ModManager.getModByName("NoSlow").isEnabled()) {
			this.movementInput.moveStrafe *= 1.0;
			this.movementInput.moveForward *= 1.0;
			this.sprintToggleTimer = 0;
		}
	...
	public void jumpMotion(double y) {
	...
	public double getFlyMotion() {
		return this.movementInput.jump ? 5.0D : (this.movementInput.sneak ? -5.0D : -0.039D);
	}
	...
	public void createAndSendC04(double posX, double posY, double posZ, boolean b) {
		this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(posX, posY, posZ, b));
	}

	public void upC04(double d, boolean b) {
		this.createAndSendC04(this.posX, this.posY + d, this.posZ, b);
	}

	public void damagePlayer() {
		if (!onGround)
			return;
		int count = 67 + (int) (getActivePotionEffect(Potion.jump) != null
				? getActivePotionEffect(Potion.jump).getAmplifier() / 3f
				: 0);
		for (int i = 0; i < count; i++) {
			mc.getNetHandler()
					.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(posX, posY + 0.045, posZ, false));
			mc.getNetHandler().addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(posX, posY, posZ, false));
			mc.getNetHandler().addToSendQueue(
					new C03PacketPlayer.C04PacketPlayerPosition(posX, posY + 0.0000000025, posZ, false));
		}
	...
	public double getBaseMoveSpeed() {
		...
		if (this.mc.thePlayer.isPotionActive(Potion.moveSpeed)) {
			baseSpeed *= 1.0D
					+ 0.2D * (double) (this.mc.thePlayer.getActivePotionEffect(Potion.moveSpeed).getAmplifier() + 1);
		}
