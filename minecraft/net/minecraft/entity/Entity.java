...
import net.AzureWare.mod.mods.COMBAT.Hitbox;
...
public abstract class Entity implements ICommandSender
{
    ...
    {
    	if (Client.INSTANCE) {
            if (this.noClip || (ModManager.getModByName("Freecam").isEnabled() && this instanceof EntityPlayerSP) || (ModManager.getModByName("Phase").isEnabled() && this instanceof EntityPlayerSP))
            {
            ...
            else
            {
                ...
                double d4 = y;
                double d5 = z;
                boolean flag = this.onGround && (this.isSneaking() && !ModManager.getModByName("Scaffold").isEnabled()) && this instanceof EntityPlayer || ModManager.getModByName("SafeWalk").isEnabled();

                if (flag || (event.safe && this.onGround))
                {
                    double d6;
            	...
            	EventStep es = new EventStep(true, (double) this.stepHeight);
            	if (this == Minecraft.getMinecraft().thePlayer)
    			EventManager.call(es);
                // == Minecraft.getMinecraft().thePlayer;

    			if (this.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z) && !es.isCancelled()
    					&& es.getStepHeight() > 0.0D) {
    				double d11 = x;
    				double d7 = y;
    				double d8 = z;
    				...
    				if (d11 * d11 + d8 * d8 >= x * x + z * z) {
    					...
    					EventStep es2 = new EventStep(false, (double) this.stepHeight, 1.0D + y);
    	            	if (this == Minecraft.getMinecraft().thePlayer)
 ...
 //   protected abstract void es(boolean b, float stepHeight2);
    ...
    public static boolean isOnGround(double height) {
    	if (!Client.INSTANCE) return true;
