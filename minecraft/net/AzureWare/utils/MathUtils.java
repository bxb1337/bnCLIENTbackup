...
public class MathUtils {
		...
		for (double i = 0.0D; i < max; i += 0.01D) {
			for (int offset : new int[] { -2, -1, 0, 1, 2 }) {
				if (Minecraft.getMinecraft().theWorld.getCollidingBoundingBoxes(Minecraft.getMinecraft().thePlayer,
						Minecraft.getMinecraft().thePlayer.getEntityBoundingBox().offset(
								Minecraft.getMinecraft().thePlayer.motionX * offset, i,
								Minecraft.getMinecraft().thePlayer.motionZ * offset))
						.size() > 0) {
					return i - 0.01D;
				}
			}
	...
	public static float randomFloatMaxMin(float a1, float a2) {
		Random v1 = new Random();
		float v2 = a2 - a1;
		float v3 = v1.nextFloat() * v2;
		return v3 + a1;
	}
