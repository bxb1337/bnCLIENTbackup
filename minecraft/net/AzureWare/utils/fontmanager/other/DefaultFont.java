...
package net.AzureWare.utils.fontmanager.other;
...
public class DefaultFont {
	...
	private void createFont(Object font, float size) {
		...
		try {
			this.theFont = font instanceof Font ? (Font) font
					: (font instanceof File ? Font.createFont(0, (File) font).deriveFont(size)
							: (font instanceof InputStream ? Font.createFont(0, (InputStream) font).deriveFont(size)
									: (font instanceof String ? new Font((String) font, 0, Math.round(size))
											: new Font("Verdana", 0, Math.round(size)))));
		...
		while (i < this.endChar) {
			...
			if ((x += (float) this.theMetrics.stringWidth(Character.toString((char) i)) + 2.0f) >= (float) (250
					- this.theMetrics.getMaxAdvance())) {
				x = 5.0f;
	...
	private void drawChar(char character, float x, float y) throws ArrayIndexOutOfBoundsException {
		...
		this.drawTexturedModalRect(x, y, this.xPos[character - this.startChar], this.yPos[character - this.startChar],
				(float) bounds.getWidth() + 1.0f,
				(float) bounds.getHeight() + (float) this.theMetrics.getMaxDescent() + 1.0f);
	}
			...
			if (text.charAt(i) == '\u00a7' && i + 1 < text.length()) {
				...
				if ((colorCode = "0123456789abcdefklmnorg".indexOf(oneMore)) < 16) {
					try {
						...
						GL11.glColor4f((float) ((float) (newColor >> 16) / 255.0f),
								(float) ((float) (newColor >> 8 & 255) / 255.0f),
								(float) ((float) (newColor & 255) / 255.0f), (float) alpha);
	...
	public String wrapFormattedStringToWidth(String s, float width) {
		...
		String split2 = String.valueOf(String.valueOf(this.getFormatFromString(split)))
	...
	public static enum FontType {
		EMBOSS_BOTTOM("EMBOSS_BOTTOM", 0), EMBOSS_TOP("EMBOSS_TOP", 1), NORMAL("NORMAL",
				2), OUTLINE_THIN("OUTLINE_THIN", 3), SHADOW_THICK("SHADOW_THICK", 4), SHADOW_THIN("SHADOW_THIN", 5);
