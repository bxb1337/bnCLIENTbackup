...
public final class LBRotate {
    ...
    public static double ll(Entity entity){
        float[] arrf = LBRotate.l(entity);
        if (arrf == null) {
            return 0.0;
        }
        return LBRotate.l(arrf[0], arrf[1]);
    }

    public static double l(float f, float f2){
        return Math.sqrt((double)(Math.pow((double)Math.abs((double)LBRotate.l((double)(ll1[0] % 360.0f), (double)f)), (double)2.0) + Math.pow((double)Math.abs((double)LBRotate.l((double)ll1[1], (double)f2)), (double)2.0)));
    }
    ...
    @EventTarget
    public static void l(EventPacket packet){
        if (packet.getPacket() instanceof C03PacketPlayer) {
            C03PacketPlayer c03PacketPlayer = (C03PacketPlayer)packet.getPacket();
            ...
            if ((c03PacketPlayer).getRotating()) {
                ll1 = new float[]{(c03PacketPlayer).getYaw(), (c03PacketPlayer).getPitch()};
            }
        }
    }
    ...
    @EventTarget
    public static void update(EventUpdate e){
        if (l && ++l11 > 15) {
            LBRotate.l1();
        }
    ...
    public static float[] l(float[] arrf, float[] arrf2, float f){
        double d = LBRotate.l((double)arrf2[0], (double)arrf[0]);
        double d2 = LBRotate.l((double)arrf2[1], (double)arrf[1]);
        float[] arrf3 = arrf;
    ...
    public static void l(Entity entity, boolean bl, boolean bl2, float f){
        ...
        if (f2 < 0.1f) {
            arrf = LBRotate.l(LBRotate.l(entity.getEntityBoundingBox()), true);
            f3 = arrf[0];
            f7 = arrf[1];
        }
        if (bl) {
        ...
        arrf = LBRotate.l(new float[]{entityPlayerSP.rotationYaw, entityPlayerSP.rotationPitch}, new float[]{f3, f7}, 10 + new Random().nextInt(6));
        if (arrf == null) {
            return;
        }
    ...
    public static float[] l(Entity entity) {
        ...
        return LBRotate.l(LBRotate.l(entity.getEntityBoundingBox(), false), true);
    }

    public static float[] l(Vec3 vec3, boolean bl){
        Vec3 vec32 = LBRotate.ll();
        if (bl) {
    ...
    public static void l(BlockPos blockPos){
        ...
        LBRotate.l1(mc.thePlayer.rotationYaw + MathHelper.wrapAngleTo180_float((float)(f - mc.thePlayer.rotationYaw)), mc.thePlayer.rotationPitch + MathHelper.wrapAngleTo180_float((float)(f2 - mc.thePlayer.rotationPitch)));
    }
}