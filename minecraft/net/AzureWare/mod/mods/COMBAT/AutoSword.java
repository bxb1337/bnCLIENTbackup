...
import net.AzureWare.utils.InventoryUtils;
...
public class AutoSword extends Mod {
	...
	private void onUpdate(EventUpdate event) {
		...
		if (this.mc.thePlayer.ticksExisted % 7 == 0) {
			...
			if (isInHBSlot) {
				if (InventoryUtils.getItemBySlotID(0) != null) {
					if (InventoryUtils.getItemBySlotID(0).getItem() instanceof ItemSword) {
						isInHBSlot = this.getSwordDamage(InventoryUtils.getItemBySlotID(0)) >= this
								.getSwordDamage(this.bestSword);
					}
				} else {
					isInHBSlot = false;
				}
			...
			if (this.shouldSwitch && this.timer.isDelayComplete(1000)) {
	...
	private ItemStack getBestItem(Class<? extends Item> itemType, Comparator comparator) {
		Optional<ItemStack> bestItem = this.mc.thePlayer.inventoryContainer.inventorySlots.stream().map(Slot::getStack)
				.filter(Objects::nonNull).filter(itemStack -> itemStack.getItem().getClass().equals((Object) itemType))
				.max(comparator);
